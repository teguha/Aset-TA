...
@section('card-body')
				...
				<div class="col-md-8 parent-group">
					@if (config('base.company.key') == 'patriot')
						...
						<div class="d-none">
							...
							<input type="text" class="form-control letter_no" value="{{ $letter->formated }}"
								readonly
								data-no="{{ $letter->no }}"
								data-format="{{ $letter->format }}">
						</div>
					@else
						<input type="hidden" name="letter_id" value="{{ $letter->id }}">
						<input type="text" class="form-control letter_no" value="{{ $letter->formated }}"
	...
	<div class="form-group row">
		...
		<div class="col-md-10 parent-group">
			<select name="members[]" class="form-control base-plugin--select2-ajax"
				...
				placeholder="{{ __('Pilih Beberapa') }}">
				<option value="">{{ __('Pilih Beberapa') }}</option>
				@foreach ($summary->members as $val)
					<option value="{{ $val->id }}" selected>{{ $val->name }} ({{ $val->position->name }})</option>
				@endforeach
			</select>
		</div>
	...
	<div class="form-group row">
		<label class="col-md-2 col-form-label">{{ __('Tanggal Pelaksanaan') }}</label>
		<div class="col-md-10 parent-group">
