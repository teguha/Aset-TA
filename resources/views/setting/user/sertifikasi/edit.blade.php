...
@section('action', rut($routes.'.sertifikasi.detailUpdate', $detail->id))
...
@section('modal-body')
    ...
    <div class="form-group row">
		...
		<div class="col-sm-9 parent-group">
			<input type="text" name="nama_sertif" class="form-control" placeholder="{{ __('Nama Sertifikat') }}" value="{{$detail->nama_sertif}}">
		</div>
	</div>
    <div class="form-group row">
		<label class="col-sm-3 col-form-label">{{ __('No Sertifikat') }}</label>
		<div class="col-sm-9 parent-group">
			<input type="text" name="no_sertif" class="form-control" placeholder="{{ __('No Sertifikat') }}" value="{{$detail->no_sertif}}">
		</div>
	</div>
    <div class="form-group row">
		<label class="col-sm-3 col-form-label">{{ __('Tgl Sertifikat') }}</label>
		<div class="col-sm-9 parent-group text-left">
			<input type="text" name="tgl_sertif" class="form-control base-plugin--datepicker width-200px mr-auto text-left" placeholder="{{ __('Tgl Sertifikat') }}" value="{{$detail->tgl_sertif->format('d/m/Y')}}">
		</div>
	</div>
    <div class="form-group row">
		<label class="col-sm-3 col-form-label">{{ __('Lembaga') }}</label>
		<div class="col-sm-9 parent-group">
			<input type="text" name="lembaga" class="form-control" placeholder="{{ __('Lembaga') }}" value="{{$detail->lembaga}}">
		</div>
	</div>
    <div class="form-group row">
		<label class="col-md-3 col-form-label">{{ __('Keterangan') }}</label>
		<div class="col-md-9 parent-group">
			<textarea name="description" class="form-control" placeholder="{{ __('Keterangan') }}">{{$detail->description}}</textarea>
		</div>
	</div>
    <div class="form-group row">
